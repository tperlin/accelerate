...
data['threads'] <- data['threads'] * 128

p<-ggplot(data=data, aes(x=factor(threads), y=throughput, ymin=throughput-sd, ymax=throughput+sd, fill=stm)) +
    scale_fill_brewer(palette="Paired") +
    scale_color_brewer(palette="Paired") +
    geom_bar(position=position_dodge(width=0.9), stat = "identity") + 
    geom_errorbar(width=.4,position = position_dodge(.9), colour="darkgrey") +
    labs(title="", subtitle="", caption = "", x="number of transactions", y="throughput (tx/s)", tag="") +
