...
//printf("t%d: validation failed: read-write conflict on addr:%d\n", id, read_log.addrs[k]);
					/*ORIGINAL*///atomicAdd(&(stats->nbAbortsReadWrite), 1);
					//stats[id].aborts++;
					return false; // return only when all the work is done
				}
			}
	...
	stm_stats_t *stats)
{
	...
	if(!TXValidate(timestamp, TxRecords, read_log, write_log, curr_r_hp, metadata->tp, metadata->tp, stats, tid)) /*ALTERACAO*/
		return 0;
	////stop_val1 = clock64();
	...
	do
	{
		...
		if(curr_r_hp != old_r_hp)
		{
			...
			if(TXValidate(timestamp, TxRecords, read_log, write_log, curr_r_hp, metadata->tp, old_r_hp, stats, tid) == false) /*ALTERACAO*/
			{	
				return 0;
			}
			////stop_val2 = clock64();
...
{	
	...
	// else
		/*ORIGINAL*///newtimestamp = TXAddToRecordNaive(id, TxRecords, metadata, txData.timestamp, txData.rs, txData.ws, data, stats, times);
		newtimestamp = TXAddToRecordNaive(id, TxRecords, metadata, txData.timestamp, txData.rs, txData.ws, data, stats);
