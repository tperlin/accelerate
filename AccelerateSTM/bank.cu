	...
	stm_stats_t *stats)
{
	...
	long mod = 0xFFFF;
	int rnd;
	int probRead = (prRead / 10.0) * 0xFFFF;
	...
	while (*flag != 2)
	{
		...
		do
		{
			...
			// if(stm_data->tr_state[tx_data->tr_id] == COMMITTED) {//trans ++;
			//__syncthreads();
			//	grid.sync();
			//	TX_garbage_collect(stm_data, tx_data);
...
template <typename T>
int consistency_check(struct STMData<T> *stm_data, T value)
{
	T total = 0;
	...
	for (int i = 0; i < stm_data->n_objects; i++)
	{
		...
		unsigned int addr_locator = EXTRACT_TX_LOCATOR_ADDR(stm_data->vboxes[i]);
...
int main(int argc, char *argv[])
{
	...
	*flag = 0;
	...
	// cudaEventRecord(start);
	// bank_kernel<<<gridDist, blockDist>>>(flag, seed, prRead, roSize, threadSize, dataSize, blockNum * threads_per_block, d_stm_data, d_stats);
	// cudaEventRecord(stop);
